# Got this code from https://gist.github.com/alexbozhkov/afb5f8eade2a9b20d25d
...
def scan(sentence): #no need to go all in and overflow your code with classes and objects , just go with a simple function
    ...
    words = sentence.split() #gets every item (word) one , by one from the lexicon.scan in the test
                             #remember that an empty space will mark the beginning of new item(3 91234 is "3" and "91234")
    result = [] # this is the tuple in the list that the test is looking for, and the function will return it after appending.

    for word in words:
        check_string = convert_numbers(word) # we need this to convert (if it can) a string to integer with convert_numbers() function
        ...
        if word in lexicon:
          ...
          #this is the tricky part, this final if-statement looks if a word is missing from our dict
